 ...
 */ 
	...
	// Überprüft ob ein übergebenes Byte dem Bitmuster eines Controlbytes f. Restfolge entspricht
	...
	// Parameter: gerechnete Prüfsumme, empfangene Prüfsumme
	// Rückgabe: 0 oder 1

	unsigned long m3s_checkSumXor(unsigned long pChkSum);
	// Codiert / Decodiert eine Prüfsumme (3 Byte) XOR mit dem oben definierten Code
	// Parameter: zu XORende Prüfsumme in unsigned long
	// Rückgabe: 1x XORed Prüfsumme in unsigned long

	unsigned char m3s_getProtocolOutOf(unsigned char pCheck);
	// Legt eine Bitmaske über ein übergebenes Byte und liefert das Protokoll an der URSPRÜNGLICHEN Position wieder zurück (restliche Bits 0)
	// Parameter: unsigned char Control Byte
	// Rückgabe: modifiziertes pData-Byte
